  ...
  <div x-show="createModal" style="display: none;" class="modal-backdrop" @click.self="createModal = false">
    <div class="modal-content" style="max-width: 540px; padding: 24px;">
      ...
      <form method="POST" action="{{ route('clients.store') }}">
        ...
        <!-- Meta Ad Account Selection Dropdown with Quick Fetch -->
        <div class="form-group" style="margin-bottom: 14px;">
          <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 4px;">
            <label class="form-label" for="modal_create_ad_account_id" style="margin-bottom: 0;">Meta Ad Account (Optional)</label>
                ...
                <button type="button" class="btn btn-secondary" style="font-size: 11px; padding: 4px 10px; white-space: nowrap;" :disabled="fetching || !inputId" @click="
                  ...
                  .then(data => {
                    ...
                    if(data.success && data.ad_account) {
                      ...
                      const sel = document.getElementById('modal_create_ad_account_id');
                      if(sel) {
          ...
          @if(!$hasGlobalMetaConnection)
            <div class="form-hint" style="color: #b45309; font-size: 11px; margin-top: 4px;">⚠️ Global Meta not connected. Connect in <a href="{{ route('settings.index', ['tab' => 'meta']) }}" style="color: var(--accent-blue);">Settings ➔ Meta</a>.</div>
          @else
            <div class="form-hint" style="font-size: 11px; margin-top: 4px;">Marketing data will automatically sync from this specific Meta Ad Account.</div>
          @endif
        </div>
